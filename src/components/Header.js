import Icon from '../assets/cloud.png'
import '../styles.css'
export const headerComponent = () =>{
  const header = createDomElement('header')
  addClassName(header, 'header-wrapper');

  const form = createDomElement('form')
  
  const searchWrapper = createDomElement('div')
  addClassName(searchWrapper, 'search-wrapper');

  const searchIcon = createDomElement('img')
  searchIcon.src = "data:image/svg+xml;utf8;base64,"
  addClassName(searchIcon, 'search-icon')

  const input = createDomElement('input')
  addClassName(input, 'search-bar')
  input.setAttribute('type', 'text')
  const logo = createDomElement('img')
  logo.classList.add('logo')
  logo.src = Icon

  const nav = createDomElement('nav')
  addClassName(nav, 'nav')


  searchWrapper.appendChild(input)
  searchWrapper.appendChild(searchIcon)
  

  nav.appendChild(logo)
  nav.appendChild(searchWrapper)
  header.appendChild(nav)

  document.body.appendChild(header)
} 



function addClassName(component, className){
  component.classList.add(className)
}
function createDomElement(element){
  return document.createElement(element)
}